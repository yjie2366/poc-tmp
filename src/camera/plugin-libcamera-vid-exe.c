...
int main(int argc, char **argv)
{
	...
	*/
	...
	if (!camera_pid) {
		...
		char *height = "480";
		char *cmd[] = {
			modules[0], "--width", width, "--height", height, "--framerate=60", "--inline",
			"-n", "-t", "0", "-o", "-", NULL
		};
		...
		if (!gst_pid) {
			...
			if (!control_pid) {
				...
				char *cmd[] = { modules[2], NULL };

				execvp(modules[2], cmd);
